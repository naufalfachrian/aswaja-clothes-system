 ...
 * @author Satrio
 */
public class BuktiPembayaranGridFrame extends javax.swing.JFrame implements MouseListener {

    /**
     * Creates new form CustomerGridFrame
     * @param valueFilter
     */
    public BuktiPembayaranGridFrame(String valueFilter) {
        initComponents();
    ...
    private void initDaftarPenjualan(String valueFilter) {
        ...
        items = ConnectionManager.getDefaultEntityManager().createNamedQuery("BuktiPembayaran.findAll", BuktiPembayaran.class).getResultList();
        
        for (BuktiPembayaran item : items) {
            int totalQty = 0;
            int totalBayar = 0;
            InvoicePesanan invoice =  item.getInvoicePesanan();
            for (PesananDetail pesananDetail : invoice.getPesanan().getPesananDetailList()) {
                totalQty += pesananDetail.getQty();
                totalBayar += (pesananDetail.getQty() * pesananDetail.getBarang().getHargaJualSatuan());
            }
            
            String[] rowData = new String[]{
                invoice.getKodeInvoice(),
                invoice.getPesanan().getKustomer().getNamaKustomer(),
    ...
    private List<BuktiPembayaran> items;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        BuktiPembayaran pembelian  = items.get(rowSelected);
        listener.onSelectedRow(pembelian, BuktiPembayaranGridFrame.class.getSimpleName());
        dispose();
    }
